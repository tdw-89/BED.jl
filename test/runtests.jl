...
# samples from groupnames, and intervals drawn to lie in [1, maxpos].
function random_intervals(groupnames, maxpos::Int, n::Int)
    ...
    for i in 1:n
        ...
        intervals[i] = GenomicInterval{Int}(groupnames[rand(seq_dist)],
                                     first, last, strand, i)
    end
    return intervals
end
    ...
    function write_intervals(filename, intervals)
        open(filename, "w") do out
            for interval in sort(intervals)
                println(out, interval.groupname, "\t", interval.first - 1, "\t", interval.last, "\t", interval.metadata, "\t", 1000, "\t", interval.strand)
            end
        end
